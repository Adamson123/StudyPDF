...
class PDFPage {
    ...
    async render() {
        this.pdfContainer.style.visibility = "visible"; // Ensure the container is visible before rendering
        const DPR = window.devicePixelRatio || 1;
    ...
    cancel() {
        try {
            this.pdfContainer.style.visibility = "hidden"; // Hide the container immediately
            this.renderTask?.cancel();
            this.isRendered = false;
            this.loader.style.display = "initial";
            this.pdfCanvas.remove(); // Remove the canvas from the DOM
            //  this.textLayer.remove(); // Remove the text layer from the DOM
