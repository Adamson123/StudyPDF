...
import { addOneCard, updateOneCard } from "@/redux/features/flashcardsSlice";
...
const EditFlashcards = ({
    ...
    const handleSave = () => {
        //If not editing, then adding new flashcard, else edit/update flashcard
        if (!flashcardToEdit) {
            dispatch(addOneCard({ id: flashcardsInfo.id, card: newFlashcard }));
            setShowEditFlashcards(false);
        } else {
            dispatch(
                updateOneCard({
                    id: flashcardsInfo.id,
                    cardIndex: flashcardToEdit.index,
                    card: newFlashcard,
                }),
            );
        }
                ...
                <div className="flex w-full items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-gray-500">
                            Add or edit your flashcards below. Click "Save" when
                            you're done.
                        </p>
                    </div>
                    <XButton
