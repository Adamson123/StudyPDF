...
function useRender({
  ...
  const renderPDFsOnView = async (
    ...
    if (firstVisibleIndex !== -1) {
      ...
      const renderStartIndex = Math.max(0, firstVisibleIndex - RENDER_BUFFER); // Start rendering a few pages before the first visible one
  ...
  useEffect(() => {
    (async () => {
      ...
      setPdfData({ ...pdfInfo, pdfDocument, numOfPages: pdfDocument.numPages }); // Update state with PDF document and metadata
      ...
      for (let index = 0; index < pdfPages.length; index++) {
        ...
        if (
          ...
          pdfPages.length // Check if the container doesn't already have the correct number of pages
        ) {
